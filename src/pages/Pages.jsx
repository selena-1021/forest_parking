...
import { UNITS_DEF, QR_CFGS, BASE } from '../data/store';
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f0f4f8', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ width:'100%', maxWidth:360, background:'#fff', borderRadius:24, padding:28, border:'0.5px solid #e2e8f0' }}>
        ...
        <button onClick={login} style={{ width:'100%', padding:12, borderRadius:10, border:'none', background:'#3b82f6', color:'#fff', fontSize:13, fontWeight:500, cursor:'pointer' }}>
          입장하기
        </button>
  ...
  if (done) return (
    <div style={{ minHeight:'100vh', background:'#f0f4f8', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ width:'100%', maxWidth:360, background:'#fff', borderRadius:24, padding:32, textAlign:'center', border:'0.5px solid #e2e8f0' }}>
        ...
        <p style={{ fontSize:13, color:'#64748b', lineHeight:1.7, marginBottom:24 }}>거주자가 확인 후 연락드릴 예정입니다.<br/>방문 시 등록하신 차량번호로 입차해 주세요.</p>
        <button onClick={() => state.in ? dispatch({ type:'GOTO', page:'map' }) : dispatch({ type:'GOTO', page:'visitor-form', vfHost:'' })}
          style={{ width:'100%', padding:12, borderRadius:10, border:'none', background:'#3b82f6', color:'#fff', fontSize:13, fontWeight:500, cursor:'pointer' }}>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f0f4f8', display:'flex', alignItems:'flex-start', justifyContent:'center', padding:'24px 20px 40px' }}>
      <div style={{ width:'100%', maxWidth:390, background:'#fff', borderRadius:24, padding:28, border:'0.5px solid #e2e8f0' }}>
        <div style={{ textAlign:'center', marginBottom:22 }}>
          <div style={{ width:52, height:52, background:'#dcfce7', borderRadius:14, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 14px', fontSize:24 }}>👥</div>
        ...
        {fixed ? (
          <Field label="방문 호수">
            <input value={`${host}호`} readOnly style={{ ...INP(), background:'#f8fafc', color:'#94a3b8', cursor:'not-allowed' }} />
          </Field>
        ) : (
        ...
        <Field label="차량 모델">
          <input value={model} onChange={e => setModel(e.target.value)} placeholder="예: 아반떼, 소나타" style={INP()} />
        </Field>
        <Field label="방문자 연락처" required error={errs.phone}>
        ...
        <button onClick={submit} style={{ width:'100%', padding:12, borderRadius:10, border:'none', background:'#3b82f6', color:'#fff', fontSize:13, fontWeight:500, cursor:'pointer', marginTop:8 }}>
          예약 등록하기
        </button>

        <div style={{ marginTop:16, padding:'10px 12px', background:'#f8fafc', borderRadius:10, border:'0.5px solid #e2e8f0' }}>
          <p style={{ fontSize:11, color:'#64748b', lineHeight:1.7 }}>· 원활한 주차 관리를 위해 방문 정보를 등록해 주세요.</p>
          <p style={{ fontSize:11, color:'#64748b', lineHeight:1.7 }}>· 차량번호와 방문자 연락처 일부는 별표(*)로 가려져 표시됩니다.</p>
        </div>
    ...
    <div style={{ padding:'16px 14px 0' }}>
      ...
      <div style={{ background:'#f8fafc', border:'0.5px solid #e2e8f0', borderRadius:12, padding:'12px 14px', marginBottom:16, display:'flex', alignItems:'center', justifyContent:'space-between', gap:10 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:5, flexShrink:0 }}>
          <button onClick={copyLink} style={{ padding:'6px 10px', borderRadius:8, border:'0.5px solid #3b82f6', background:'#dbeafe', color:'#1d4ed8', fontSize:11, fontWeight:500, cursor:'pointer' }}>링크 복사</button>
      ...
      <button onClick={submit} style={{ width:'100%', padding:12, borderRadius:10, border:'none', background: saved ? '#f0fdf4' : '#3b82f6', border: saved ? '0.5px solid #86efac' : 'none', color: saved ? '#15803d' : '#fff', fontSize:13, fontWeight:500, cursor:'pointer', marginTop:4 }}>
        {saved ? '✓ 등록 완료' : '예약 등록하기'}
      </button>
      ...
      {my.length > 0 && (
        <div style={{ marginTop:22, borderTop:'0.5px solid #f1f5f9', paddingTop:14 }}>
          ...
          {my.map(v => (
            <div key={v.id} style={{ background:'#f8fafc', border:'0.5px solid #e2e8f0', borderRadius:12, padding:'12px 13px', marginBottom:8, display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:8 }}>
              ...
              <button onClick={() => { if(window.confirm('이 방문 예약을 삭제하시겠어요?')) dispatch({ type:'DEL_VISIT', id:v.id }); }}
                style={{ background:'none', border:'none', cursor:'pointer', color:'#94a3b8', padding:'2px', flexShrink:0 }} title="삭제">🗑</button>
            </div>
          ))}
        </div>
      )}
...
export function MyInfoPage({ state, dispatch }) {
  const r = state.res[state.me] || {};
  const [car, setCar]     = useState(r.car || '');
...
function AdminQR({ state }) {
  ...
  useEffect(() => {
    QR_CFGS.forEach(cfg => drawQRCanvas(refs[cfg.key].current, cfg.url));
  }, []);
...
function AdminSettings({ state, dispatch }) {
  ...
  return (
    <>
      ...
      <button onClick={save} style={{ width:'100%', padding:12, borderRadius:10, border: saved ? '0.5px solid #86efac' : 'none', background: saved ? '#f0fdf4' : '#3b82f6', color: saved ? '#15803d' : '#fff', fontSize:13, fontWeight:500, cursor:'pointer', marginBottom:14 }}>
        {saved ? '✓ 저장됨' : '변경사항 저장'}
      </button>
